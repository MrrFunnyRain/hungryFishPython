...
def fillLitter():
    ...
    sX.append(800)
    sY.append(random.randint(200,550)) # 200shezhile
    sSpeed.append(random.randrange(10,20))
...
def drawScene(screen, mx, my, button):
    ...
    pygame.draw.rect(screen, RED, (150,145,530,80), 5) # title
    string1 = "HELP FISH GO HOME"
    text1 = fontBig.render(string1,0,BLACK)
    screen.blit(text1,pygame.Rect(160,165,250,80))       
...
def playGame(screen, mx, my, button):
    ...
    addBomb()
        ...
        #for i in range(numLitter): # numLitter = 1
            ...
            #fillLitter()
            #fillLitter()
        #for p in range(numPoint):                       # A way that Mr.Van Rooyen have taught
            #fillPoints()
    ...
    # Hearts set
    if (gameTime - totalTime) %  371 == 0 and 0 <=  clockCount  <= 60:
        fillHeart()                           # fill lists for heart
    
    # Bomb set
    if (gameTime - totalTime) %  341 == 0 and 0 <=  (gameTime - totalTime)  <= 60:
